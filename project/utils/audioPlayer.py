...
class AudioPlayer:
    ...
    def __record_audio(self):
        ...
        for i in tqdm(
            np.linspace(0, seconds, int(seconds * 20)),
            desc="Recording",
            unit="s",
            smoothing=0,
            bar_format="{desc} |{bar}|",
    ...
    def __transcribe_audio(self):
        ...
        result = self.modelRecorder.transcribe(
            AUDIO_PATH,
            initial_prompt="Dictated chess moves in UCI format: four characters (letter-digit-letter-digit), optionally a fifth letter for promotion [q, r, b, n]. For example: d2d4, a7a8q, c5d6n. Moves may be spoken in different languages but should be interpreted as UCI moves.",
        )
