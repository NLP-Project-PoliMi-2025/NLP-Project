...
class LSTMChessBot(_ChessBot):
    def __init__(
        ...
        self.w2v:Word2Vec = Word2Vec.load(".\model_weights\word2vec.model")
    ...
    def load_model(self, weights: str) -> SeqAnnotator:
        ...
        state_dict = torch.load("./model_weights/bilstm_ner_model.pth", weights_only=True)
    ...
    def __call__(self, past_moves: List[str], legal_moves: List[str]) -> str:
        ...
        past_moves_embeds = torch.tensor(
            [self.w2v.wv.vectors[self.moves_vocab_table[move]] for move in past_moves]
        )
        legal_moves_embeds = torch.tensor(
            [self.w2v.wv.vectors[self.moves_vocab_table[move]] for move in legal_moves]
        )
        print(legal_moves_embeds)
        print(legal_moves_embeds.shape)

        # get the model prediction
        with torch.no_grad():
            lstm_out_current, (hn, cn) = self.model.lstm.forward(past_moves_embeds.unsqueeze(0))  # Add batch dimension
            hidden = (hn.repeat(1, legal_moves_embeds.shape[0], 1),  # Repeat hidden state for each legal move
                     cn.repeat(1, legal_moves_embeds.shape[0], 1))  # Use the last hidden state as the initial hidden state for the next forward pass
            lstm_out, hidden = self.model.lstm.forward(legal_moves_embeds[:, None], hidden)  # Add batch dimension
            logits = self.model.fc.forward(lstm_out[:, -1, :])  # Use the last output of LSTM
            preds = torch.softmax(logits, dim=-1)  # Apply softmax to logits

            current_evaluation = self.model.fc.forward(lstm_out_current[:, -1, :])
            current_evaluation = torch.softmax(current_evaluation, dim=-1).squeeze()

        current_evaluation = current_evaluation.numpy()

        preds = preds[:, self.objective]  # account for 0-indexing
